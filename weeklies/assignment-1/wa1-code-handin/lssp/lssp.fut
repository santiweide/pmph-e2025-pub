...
let lss_redOp (pred2: i32 -> i32 -> bool)
  ...
  let segments_connect = 
    i32.bool (x_len > 0 && y_len > 0 && pred2 x_last y_first)

  let new_lss = 
    max (max x_lss y_lss) ((x_lcs + y_lis) * segments_connect)

  let new_lis =     
    if x_len == 0 then y_lis
    else if x_lis == x_len then x_lis + (y_lis * segments_connect)
    else x_lis

  let new_lcs = 
    if y_len == 0 then x_lcs
    else if y_lcs == y_len then y_lcs + (x_lcs * segments_connect)
    else y_lcs
